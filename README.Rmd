...
left_join(num_total, num_status) %>%
  rename(Type = type) %>%
  kable(caption="Number of locations by type") %>% 
...
# Load world 1998 provinces shapefile
provinces.shp <- st_read("world_admin_1998.dbf", quiet = TRUE) %>%
  st_transform(crs = st_crs(allsites.shp))
provinces.shp <- provinces.shp %>% filter(REGION != "Antarctica")

ggplot() + 
  geom_sf(data = provinces.shp, fill = "white", color = "black", size = .2) + 
